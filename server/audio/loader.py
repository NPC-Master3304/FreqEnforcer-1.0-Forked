...
def set_wav_root_note(
    ...
    smpl_data = struct.pack(
        ...
        0,                   # manufacturer
        0,                   # product
        int(sample_period),  # sample_period (ns)
        midi,                # midi_unity_note
        0,                   # midi_pitch_fraction
        0,                   # smpte_format
        0,                   # smpte_offset
        num_loops,           # num_sample_loops
        0,                   # sampler_data_size
    )
    ...
    if has_loop:
        smpl_data += struct.pack(
            "<6I",
            0,               # cue_point_id
            int(loop_type),  # type: 0=forward, 1=ping-pong, 2=backward
            int(loop_start), # start sample
            int(loop_end),   # end sample
            0,               # fraction
...
def set_wav_loop_points(
    ...
    """
    Write or update the smpl chunk in a WAV file to include loop points.
    Reads the existing root note from the smpl chunk if present, otherwise
    defaults to MIDI note 60 (middle C).

    Args:
        wav_path: path to the WAV file
        loop_start: loop start in samples
        loop_end: loop end in samples
        loop_type: 0=forward, 1=ping-pong, 2=backward
    """
    path = Path(wav_path)
    data = path.read_bytes()
    if len(data) < 12 or data[0:4] != b"RIFF" or data[8:12] != b"WAVE":
    ...
    # Extract sample rate from fmt chunk
    sr = 44100
    midi = 60
    offset = 12
    while offset + 8 <= len(data):
        ...
        if chunk_id == b"fmt " and chunk_size >= 16:
            sr = struct.unpack_from("<I", data, start + 4)[0]
        elif chunk_id == b"smpl" and chunk_size >= 36:
            midi = struct.unpack_from("<I", data, start + 12)[0]
            midi = max(0, min(127, midi))
        if end > len(data):
            break
        offset = end + (chunk_size & 1)

    set_wav_root_note(wav_path, midi, sr, loop_start, loop_end, loop_type)
